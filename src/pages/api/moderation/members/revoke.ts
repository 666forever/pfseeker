...
import { findMembershipByRevokeToken } from "@/server/services/moderation-membership-actions";
import {
...
export const POST: APIRoute = async (context) => {
  try {
    ...
    const target = await findMembershipByRevokeToken({
      repository: access.repository,
      config: await getAuthConfig(context.locals),
      token: data.membershipToken,
    });
