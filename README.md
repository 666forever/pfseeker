...
Phase 13 deployed and owner-bootstrap verified; full moderation workflow verification pending. It adds protected moderator/owner workflows for submission review, metadata correction, required taxonomy assignment before publication, approval/publication, rejection, archive, taxonomy management, membership management, and append-only moderation history. Migration `0006_moderation_and_publishing.sql` is applied in production, owner bootstrap was manually verified, durable owner access persisted after sign-out/sign-in, and production currently has one active owner membership. Reports remain deferred and no report tables or `/moderation/reports` route are implemented. Taxonomy management, approval, rejection, archive, moderator creation/revocation, and the full moderation workflow are not yet manually production-tested. See `MODERATION.md`.
